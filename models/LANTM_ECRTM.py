...
class LANTM_ECRTM(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.weight_loss_ECR = params["weight_loss_ECR"]
        self.beta_temp = params["beta_temp"]
    ...
    def compute_loss(self, bows, labels):
        ...
        log_recon_class = (label_topic_dist @ beta).add(1e-10).log()
        L_sem = -(self.pseudo_doc_per_class * log_recon_class).sum(1).mean()

        loss_TM = recon_loss + kld
        loss_ECR = self._get_loss_ECR()
        loss = (loss_TM +
                self.weight_loss_ECR * loss_ECR +
                self.gamma1 * L_OT +
                self.gamma2 * L_sem)
    ...
    def _get_beta(self):
        dist = self._pairwise_euclidean_distance(self.topic_embeddings, self.word_embeddings)
        beta = F.softmax(-dist / self.beta_temp, dim=0)
        return beta

    def _get_label_topic_mat(self):
        cost = self._pairwise_euclidean_distance(self.label_embeddings, self.topic_embeddings)
        a = self.label_dist
    ...
    def _get_loss_ECR(self):
        cost = self._pairwise_euclidean_distance(self.topic_embeddings, self.word_embeddings)
        a = torch.ones((self.num_topics,), device=cost.device) / self.num_topics
        b = torch.ones((self.vocab_size,), device=cost.device) / self.vocab_size
        _, loss_ECR = self._optimal_transport(a, b, cost)
        return loss_ECR

    @staticmethod
    def _pairwise_euclidean_distance(x, y):
        distance_mat = (x.unsqueeze(-2) - y).pow(2).sum(-1)
        return distance_mat
